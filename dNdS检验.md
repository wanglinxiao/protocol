...
| ---------- | -------------- | ---- | ------------------------------------------------------------ | ------------------------------------------------- | ------------------- |
| Zhen  Wang | fasta2phy.pl   | perl | 将fasta格式的CDS比对转化为phylip格式（用Bioperl的AlignIO实现） | CDS多重比对(fasta)                                | CDS多重比对(phylip) |
| Zhen Wang  | prepare_ctl.pl | perl | 根据写好的codeml控制文件模板，批量产生控制文件               | 控制文件模板(ctl)，所有CDS比对文件，所有CDS进化树 | 所有控制文件        |
...
![img](https://github.com/zhenwang100/protocal/raw/master/images/clip_image001.jpg)
